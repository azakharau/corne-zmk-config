...
void draw_battery(lv_obj_t *canvas, const struct conn_status_state *state) {
    ...
    canvas_draw_rect(canvas, 0, 2, 29, 12, &foreground_dsc);
    canvas_draw_rect(canvas, 1, 3, 27, 10, &background_dsc);
    canvas_draw_rect(canvas, 2, 4, (state->battery + 2) / 4, 8, &foreground_dsc);
    canvas_draw_rect(canvas, 30, 5, 3, 6, &foreground_dsc);
    canvas_draw_rect(canvas, 31, 6, 1, 4, &background_dsc);

    if (state->charging) {
        lv_draw_image_dsc_t image_dsc;
        lv_draw_image_dsc_init(&image_dsc);
        canvas_draw_img(canvas, 9, -1, &bolt, &image_dsc);
    }
}
